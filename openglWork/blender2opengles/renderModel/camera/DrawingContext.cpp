...
DrawingContext::DrawingContext(cv::Size frameSize, const CameraCalibration& c)
  : m_isBackgroundTextureInitialized(false)
  , m_isFurnishTextureInitialized(false)
  , m_isTigerTextureInitialized(false)
  , m_calibration(c)
...
void DrawingContext::updateFurnishImage()
{
...
void DrawingContext::updateTigerImage()
{
...
void DrawingContext::setWidth(unsigned _width){
	m_width = _width;
	//~ LOG_INFO("updating width to %d", m_width);
}

void DrawingContext::setHeight(unsigned _height){
	m_height = _height;
	//~ LOG_INFO("updating height to %d", m_height);
}

bool DrawingContext::isWindowUpdated(){
	//~ LOG_INFO("returning is window updated...");
	return m_isWindowUpdated;
}

void DrawingContext::setObjectToDraw(int objectId){
	m_objectToDraw = objectId;
}

int DrawingContext::objectToDraw(){
...
void DrawingContext::destroyTexture() {
...
void DrawingContext::renderToFrame(){
	...
	getObjectTexture(m_textureId[1],m_furnishImage);
	getObjectTexture(m_textureId[2],m_tigerImage);
	getObjectTexture(m_textureId[3],m_tigerImage);
	...
	//~ int glewWidth =  1024;
	//~ int glewHeight = 768;
	int glewWidth =  m_tigerImage.cols;
	int glewHeight = m_tigerImage.rows;
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, glewWidth, glewHeight);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, m_depthrenderbuffer);  
	glBindRenderbuffer(GL_RENDERBUFFER, 0);

	glBindTexture(GL_TEXTURE_2D, m_textureId[3]);
	glTexImage2D(GL_TEXTURE_2D, 0,GL_RGB, glewWidth, glewHeight, 0,GL_RGB, GL_UNSIGNED_BYTE, NULL);
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D, m_textureId[3], 0);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, m_depthrenderbuffer);  	 
	...
	glPushAttrib(GL_VIEWPORT_BIT); // viewport bit
	glViewport(0,0,glewWidth,glewHeight);

	drawCameraFrame(m_textureId[0],glewWidth,glewHeight,false);
	...
	while(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE){
		...
		std::cout << "rendering to frame " << std::endl;
	}
...
// !!!!!!!!!! change BGR to RGB !!!!!!!!!!!!!!
void DrawingContext::drawCameraFrame(unsigned int texName,int w, int h, bool flipped)
{
	//~ orthogonalStart();
	
	GLfloat bgTextureVertices[] = { 0, 0, 0, w, 0, 0, 0, h, 0, w, h, 0 };
	if (flipped){
		bgTextureVertices[0]=w;
		//~ bgTextureVertices[1]=0;
		//~ bgTextureVertices[2]=0;
		bgTextureVertices[3]=0;
...
void DrawingContext::buildProjectionMatrix(const CameraCalibration& calibration, int screen_width, int screen_height, Matrix44& projectionMatrix)
{
...
// !!!!!!!!!!!!!!! change RGB to BGR
void DrawingContext::drawFurnish(float lPosition[])
{	
